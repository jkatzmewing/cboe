...
monster_record_type return_monster_template(unsigned char store) {
  ...
  monst.m_health =
      monst.health; // in scenario file, health is stored in health field
  monst.max_mp = 0;
  ...
  if ((PSD[SDF_LEGACY_SCENARIO] == 1) &&
      (monst.spec_skill ==
       MONSTER_PETRIFYING_TOUCH)) // legacy scenario, petrifying touch is (was),
                                  // in fact, disease touch
...
short difficulty_adjust() {
  ...
  if (scenario.adjust_diff >
      0) // don't automatically adjust difficulty flag activated
    return 1;
...
void create_wand_monst() {
  ...
  if (overall_mode == MODE_OUTDOORS)
    if (outdoors[party.i_w_c.x][party.i_w_c.y].wandering[r1].isNull() ==
        false) {
      r2 = get_ran(1, 0, 3);
      while ((point_onscreen(
                  outdoors[party.i_w_c.x][party.i_w_c.y].wandering_locs[r2],
                  party.p_loc.toLocal()) == true) &&
             (num_tries++ < 100))
        r2 = get_ran(1, 0, 3);
      if (is_blocked(
              outdoors[party.i_w_c.x][party.i_w_c.y].wandering_locs[r2]) ==
  ...
  if (overall_mode !=
      MODE_OUTDOORS) // won't place wandering is more than 50 monsters
    if ((c_town.town.wandering[r1].isNull() == false) &&
        (c_town.countMonsters() <= 50) &&
        (party.m_killed[c_town.town_num] < c_town.town.max_num_monst)) {
      r2 = get_ran(1, 0, 3);
      while ((point_onscreen(c_town.town.wandering_locs[r2], c_town.p_loc) ==
              true) &&
             (loc_off_act_area(c_town.town.wandering_locs[r2]) == false) &&
             (num_tries++ < 100))
        r2 = get_ran(1, 0, 3);
      for (i = 0; i < 4; i++) {
        if (c_town.town.wandering[r1].monst[i] != 0) { // place a monster
          ...
          r3 = get_ran(1, 0, 3);
          if ((r3 >= 2) && (i == 3) &&
              (is_blocked(p_loc) == false)) // place extra monsters?
            c_town.placeMonster(c_town.town.wandering[r1].monst[i], p_loc);
        }
      }
    }
...
void place_outd_wand_monst(location where, out_wandering_type group,
                           short forced) {
  short i = 0, j = 0;
  location l;
  ...
  while (i < 10) {
    ...
    if ((party.out_c[i].exists == false) || ((i == 9) && (forced > 0))) {
      if ((sd_legit(group.end_spec1, group.end_spec2) == true) &&
          (PSD[group.end_spec1][group.end_spec2] > 0))
        return;
    ...
    for (i = 0; i < T_M; i++)
      if ((c_town.monst.dudes[i].active != 0) &&
        ...
        if ((c_town.monst.dudes[i].active == 2) ||
          ...
          if (((c_town.monst.dudes[i].attitude == 0) ||
               (monst_target[i] == 6)) &&
              (c_town.hostile == 0)) {
            if (c_town.monst.dudes[i].mobile ==
                true) { // OK, it doesn't see the party or
              // isn't nasty, and the town isn't totally hostile.
              if ((c_town.monst.dudes[i].attitude % 2 != 1) ||
                  (get_ran(1, 0, 1) == 0)) {
                rand_move(i);
              } else
                seek_party(i, c_town.monst.dudes[i].m_loc, c_town.p_loc);
            }
          }
          if ((c_town.monst.dudes[i].attitude > 0) || (c_town.hostile == 1)) {
            if ((c_town.monst.dudes[i].mobile == true) &&
                (monst_target[i] != 6)) {
              l1 = c_town.monst.dudes[i].m_loc;
              l2 = (monst_target[i] <= 6)
                       ? c_town.p_loc
                       : c_town.monst.dudes[target - 100].m_loc;

              if ((c_town.monst.dudes[i].m_d.morale < 0) &&
                  (c_town.monst.dudes[i].m_d.spec_skill != MONSTER_MINDLESS) &&
        ...
        if ((c_town.monst.dudes[i].active == 1) &&
          ...
          for (j = 0; j < T_M; j++)
            if ((c_town.monst.dudes[j].active == 2) &&
                ((dist(c_town.monst.dudes[i].m_loc,
                       c_town.monst.dudes[j].m_loc) <= 5) == true))
              c_town.monst.dudes[i].active = 2;
        }
      }
  if (overall_mode == MODE_OUTDOORS) {
...
Boolean monst_hate_spot(short which_m, location *good_loc) {
  ...
  if ((misc_i[loc.x][loc.y] & 224) ||
      ...
      || ((c_town.explored[loc.x][loc.y] & 32) &&
          (c_town.monst.dudes[which_m].m_d.radiate_1 !=
           MONSTER_RADIATE_ICE_FIELDS) &&
          ((c_town.monst.dudes[which_m].m_d.immunities & 32) ==
           0)) // hate ice wall?
      || ((c_town.explored[loc.x][loc.y] & 4) &&
          (c_town.monst.dudes[which_m].m_d.radiate_1 !=
           MONSTER_RADIATE_FIRE_FIELDS) &&
          ((c_town.monst.dudes[which_m].m_d.immunities & 8) ==
           0)) // hate fire wall?
      || ((c_town.explored[loc.x][loc.y] & 16) &&
          (c_town.monst.dudes[which_m].m_d.radiate_1 !=
           MONSTER_RADIATE_STINKING_CLOUDS) &&
          ((c_town.monst.dudes[which_m].m_d.immunities & 3) ==
           0)) // hate stink cloud?
      || ((c_town.explored[loc.x][loc.y] & 128) &&
          (c_town.monst.dudes[which_m].m_d.radiate_1 !=
           MONSTER_RADIATE_SLEEP_FIELDS) &&
          ((c_town.monst.dudes[which_m].m_d.immunities & 3) ==
           0)) // hate sleep cloud?
      || ((c_town.explored[loc.x][loc.y] & 2) &&
          (c_town.monst.dudes[which_m].m_d.radiate_1 !=
           MONSTER_RADIATE_SHOCK_FIELDS) &&
          ((c_town.monst.dudes[which_m].m_d.immunities & 3) ==
           0)) // hate shock cloud?
      || (((c_town.monst.dudes[which_m].m_d.mu > 0) ||
           (c_town.monst.dudes[which_m].m_d.cl > 0)) &&
...
short monst_pick_target(short which_m) {
  ...
  if (monst_target[which_m] >= 100) {
    if (((cur_monst->attitude % 2 == 1) &&
         (c_town.monst.dudes[monst_target[which_m] - 100].attitude ==
          cur_monst->attitude)) ||
        ((cur_monst->attitude % 2 == 0) &&
         (c_town.monst.dudes[monst_target[which_m] - 100].attitude % 2 == 0)))
  ...
  if ((is_combat()) && (cur_monst->attitude % 2 == 1)) {
    if (spell_caster < 6)
      if ((get_ran(1, 1, 5) < 5) &&
          (monst_can_see(which_m, pc_pos[spell_caster]) == true) &&
          (adven[spell_caster].isAlive()))
        return spell_caster;
    if (missile_firer < 6)
      if ((get_ran(1, 1, 5) < 3) &&
          (monst_can_see(which_m, pc_pos[missile_firer]) == true) &&
...
short monst_pick_target_monst(creature_data_type *which_m) {
  ...
  for (i = 0; i < T_M; i++) {
    if ((c_town.monst.dudes[i].active > 0) && // alive
        (((which_m->attitude % 2 == 1) &&
          (c_town.monst.dudes[i].attitude % 2 == 0)) ||
         ((which_m->attitude % 2 == 0) &&
          (c_town.monst.dudes[i].attitude % 2 == 1)) ||
         ((which_m->attitude % 2 == 1) &&
          (c_town.monst.dudes[i].attitude !=
           which_m->attitude))) && // they hate each other
...
short switch_target_to_adjacent(short which_m, short orig_target) {
  ...
  if (c_town.monst.dudes[which_m].attitude % 2 == 0) {
    if (orig_target >= 100)
      if ((c_town.monst.dudes[orig_target - 100].active > 0) &&
          (monst_adjacent(c_town.monst.dudes[orig_target - 100].m_loc,
                          which_m) == true))
        return orig_target;
    for (i = 0; i < T_M; i++)
      if ((c_town.monst.dudes[i].active > 0) &&
  ...
  if ((is_combat()) && (orig_target < 6))
    if ((adven[orig_target].isAlive()) &&
        (monst_adjacent(pc_pos[orig_target], which_m) == true))
      return orig_target;
  if (orig_target >= 100)
    if ((c_town.monst.dudes[orig_target - 100].active > 0) &&
        (monst_adjacent(c_town.monst.dudes[orig_target - 100].m_loc, which_m) ==
         true))
      return orig_target;
  ...
  if (is_combat())
    for (i = 0; i < 6; i++)
      if ((adven[i].isAlive()) &&
          (monst_adjacent(pc_pos[i], which_m) == true) &&
  ...
  // OK. Now if this monster has PCs adjacent, pick one at randomn and hack.
  // Otherwise, stick with orig. target.
  for (i = 0; i < 6; i++)
  ...
  while ((num_adj > 1) || (adven[i].isAlive() == false) ||
         (monst_adjacent(pc_pos[i], which_m) == false)) {
    if (adven[i].isAlive() && (monst_adjacent(pc_pos[i], which_m) == true))
      num_adj--;
    i++;
  }
  ...
  if (acted_yet == false) {
    ...
    for (j = 0; j < 3; j++) {
      ...
      if ((loc_off_act_area(store_loc) == false) &&
          (can_see(c_town.monst.dudes[i].m_loc, store_loc, 0) < 5)) {
...
Boolean combat_move_monster(short which, location destination) {
  if (monst_can_be_there(destination, which) == false)
    ...
    if ((monst_target[which] != 6) &&
        (point_onscreen(destination, center) == true)) {
      if (is_combat())
        move_sound(combat_terrain[destination.x][destination.y],
                   (short)c_town.monst.dudes[which].m_d.ap);
      else
        move_sound(t_d.terrain[destination.x][destination.y],
                   (short)c_town.monst.dudes[which].m_d.ap);
    }
...
// Looks at all spaces within 2, looking for a spot which is clear of nastiness
// and beings returns {0,0} if none found THIS MAKES NO ADJUSTMENTS FOR BIG
// MONSTERS!!!
...
Boolean outdoor_move_monster(short num, location dest) {
  ...
  if ((outd_is_blocked(dest) == false) && (outd_is_special(dest) == false) &&
    ...
    party.out_c[num].direction = set_direction(party.out_c[num].m_loc, dest);
...
Boolean monster_placid(short m_num) {
  if ((c_town.monst.dudes[m_num].attitude == 0) ||
      ((c_town.monst.dudes[m_num].attitude == 2) &&
...
// This damages a monster by any fields it's in, and destroys any barrels or
// crates it's stiing on.
    ...
    for (j = 0; j < c_town.monst.dudes[which_monst].m_d.y_width; j++)
      if (c_town.monst.dudes[which_monst].active > 0) {
        ...
        if (is_force_wall(where_check.x, where_check.y)) {
          r1 = get_ran(3, 1, 6);
          if (c_town.monst.dudes[which_monst].m_d.radiate_1 !=
              MONSTER_RADIATE_SHOCK_FIELDS)
        ...
        if (is_ice_wall(where_check.x, where_check.y)) {
          r1 = get_ran(3, 1, 6);
          // if (c_town.monst.dudes[which_monst].m_d.spec_skill !=
          // MONSTER_PARALYSIS_RAY)//old exile 3 leftover
          if (c_town.monst.dudes[which_monst].m_d.radiate_1 !=
              MONSTER_RADIATE_ICE_FIELDS)
        ...
        if ((is_web(where_check.x, where_check.y)) &&
            (which_m->m_d.m_type != MONSTER_TYPE_BUG)) {
...
Boolean monst_check_special_terrain(location where_check, short mode,
                                    short which_monst)
// mode; // 1 - town 2 - combat
{
  ...
  Boolean do_look = false; // If becomes true, terrain changed, so need to
                           // update what party sees
  creature_data_type *which_m;
  short ter_abil;
  ...
  if ((mode > 0) && (ter_abil >= TER_SPEC_CONVEYOR_NORTH) &&
    ...
    if (((ter_abil == TER_SPEC_CONVEYOR_NORTH) &&
         (where_check.y > from_loc.y)) ||
        ((ter_abil == TER_SPEC_CONVEYOR_EAST) &&
         (where_check.x < from_loc.x)) ||
        ((ter_abil == TER_SPEC_CONVEYOR_SOUTH) &&
         (where_check.y < from_loc.y)) ||
        ((ter_abil == TER_SPEC_CONVEYOR_WEST) &&
         (where_check.x > from_loc.x))) {
      return false;
  ...
  // begin determining guts, which determines how enthused the monst is about
  // entering nasty barriers
  if ((which_m->m_d.mu > 0) || (which_m->m_d.cl > 0))
    mage = true;
  if (which_m->m_d.spec_skill == MONSTER_MINDLESS)
    guts = 20;
  else
    guts = get_ran(1, 1, (which_m->m_d.level / 2));
  ...
  if ((is_antimagic(where_check.x, where_check.y)) && (mage == true))
    return false;
  if ((is_fire_wall(where_check.x, where_check.y)) &&
      (which_m->m_d.radiate_1 !=
       MONSTER_RADIATE_FIRE_FIELDS)) { //&& (which_m->m_d.spec_skill !=
                                       //MONSTER_PERMANENT_MARTYRS_SHIELD))
                                       ////old Exile 3 leftover
    if (guts < 3)
      return false;
  }
  if ((is_force_wall(where_check.x, where_check.y)) &&
      (which_m->m_d.radiate_1 != MONSTER_RADIATE_SHOCK_FIELDS)) {
    if (guts < 4)
      return false;
  }
  if ((is_ice_wall(where_check.x, where_check.y)) &&
      (which_m->m_d.radiate_1 !=
       MONSTER_RADIATE_ICE_FIELDS)) { //&& (which_m->m_d.spec_skill !=
                                      //MONSTER_PARALYSIS_RAY)) //old Exile 3
                                      //leftover
    if (guts < 5)
      return false;
  }
  if ((is_sleep_cloud(where_check.x, where_check.y)) &&
      (which_m->m_d.radiate_1 != MONSTER_RADIATE_SLEEP_FIELDS)) {
    if (guts < 8)
      return false;
  }
  if (is_blade_wall(where_check.x, where_check.y)) {
  ...
  if ((is_scloud(where_check.x, where_check.y)) &&
      (which_m->m_d.radiate_1 != MONSTER_RADIATE_STINKING_CLOUDS)) {
    if (guts < 4)
      return false;
  }
  if ((is_web(where_check.x, where_check.y)) &&
  ...
  if (is_fire_barrier(where_check.x, where_check.y)) {
    if ((which_m->attitude % 2 == 1) &&
        (get_ran(1, 0, 100) < (which_m->m_d.mu * 10 + which_m->m_d.cl * 4))) {
      play_sound(60);
  ...
  if (monster_placid(
          which_monst) && // monsters don't hop into bed when things are calm
      (scenario.ter_types[ter].picture == 143))
    can_enter = false;
  if ((scenario.ter_types[ter].picture <= 212) &&
      (scenario.ter_types[ter].picture >= 207))
    can_enter = false;
  if (ter == 90) {
  ...
  case TER_SPEC_CHANGE_WHEN_STEP_ON:
    ...
    if (!(monster_placid(which_monst))) {
      ...
      combat_terrain[where_check.x][where_check.y] =
          scenario.ter_types[ter].flag1;
      do_look = true;
  ...
  case TER_SPEC_DOES_FIRE_DAMAGE:
    if (c_town.monst.dudes[which_monst].m_d.immunities & 8) // fire immune?
      ...
      return false;
    break;
    // todo : add ice/magic damaging terrains for new format scenarios
    // (compatibility)
  }
...
void forced_place_monster(unsigned char which, location where) {
  ...
  while ((free_spot == false) &&
         (i < T_M)) { // is there already an available spot ?
    if (c_town.monst.dudes[i].active == 0)
      free_spot = true;
    i++;
  }
  ...
  if (free_spot == false) { // get a random monster that has no life flag or
                            // break after 1200 try.
    i = 0;
    do {
      r1 = get_ran(1, 0, 59);
      i++;
    } while (((c_town.monst.dudes[r1].monst_start.spec1 != 0) ||
              (c_town.monst.dudes[r1].monst_start.spec2 != 0)) &&
             i < 1200);
    if (i == 1200)
      give_error("Couldn't force place the monster, make sure all monsters "
                 "don't have a life flag.",
                 "", 0);
    else
...
void creature_data_type::slow(short how_much) {
  adjustMagic(&how_much);
  m_d.status[STATUS_HASTE_SLOW] =
      minmax(-8, 8, m_d.status[STATUS_HASTE_SLOW] - how_much);
  monst_spell_note(number, (how_much == 0) ? 10 : 2);
}
void creature_data_type::curse(short how_much) {
  adjustMagic(&how_much);
  m_d.status[STATUS_BLESS_CURSE] =
      minmax(-8, 8, m_d.status[STATUS_BLESS_CURSE] - how_much);
...
void creature_data_type::disease(short how_much) {
  adjustMagic(&how_much);
  m_d.status[STATUS_DISEASE] =
      minmax(-8, 8, m_d.status[STATUS_DISEASE] + how_much);
...
// 11 is sleep, 12 is paralysis
{
  ...
  /*	if ((which_status == STATUS_ASLEEP) && (number >= 138) && (number <=
     142))	return;//old windows only checks if ((which_status ==
     STATUS_ASLEEP) && (m_d.m_type == MONSTER_TYPE_UNDEAD)) return;*/
  if ((which_status == STATUS_ASLEEP) && ((m_d.m_type == MONSTER_TYPE_UNDEAD) ||
                                          (m_d.m_type == MONSTER_TYPE_SLIME) ||
                                          (m_d.m_type == MONSTER_TYPE_STONE)))
    return;
  r1 = get_ran(1, 0, 100);
  if (m_d.immunities & 1)
    r1 = r1 * 2; // magic resistant?
  if (m_d.immunities & 2)
    r1 = 200; // magic immune?
  r1 += penalty;
  if (which_status == STATUS_ASLEEP)
    r1 -= 25;
  if (which_status == STATUS_PARALYZED)
    r1 -= 15;
  if ((which_status == STATUS_ASLEEP) &&
      (m_d.spec_skill ==
       MONSTER_BREATHES_SLEEP_CLOUDS)) // sleep clouds breathing monsters are
                                       // immuned to sleep
    return;
  ...
  if (r1 > charm_odds[m_d.level / 2]) {
    ...
    if (which_status == 0) {
      ...
      if (which_status == STATUS_ASLEEP)
        monst_spell_note(number, 28);
      if (which_status == STATUS_PARALYZED)
        monst_spell_note(number, 30);
    }
  }
}
...
void creature_data_type::record() {
  ...
  } else if ((r1 > charm_odds[m_d.level / 2]) ||
             (m_d.spec_skill == MONSTER_SPLITS) ||
...
Boolean summon_monster(unsigned char which, location where, short duration,
...
// to put monster
{
  ...
  if ((is_town()) || (monsters_going)) {
    // Ooooh ... mondo kludge. Need to find caster's attitude to give it to
    // monst.
    loc = find_clear_spot(where, 0);
    if (loc.x == 0)
      return false;
  } else {
    // pc may be summoning using item, in which case where will be pc's space,
    // so fix
    if (pc_there(where) < 6) {
  ...
  if (which > 0) { // monster here for good
    ...
    c_town.monst.dudes[spot].summoned = 0;
...
void current_town_type::activateMonsters(short code) {
  ...
  for (i = 0; i < T_M; i++)
    if (monst.dudes[i].monst_start.spec_enc_code == code) {
      if ((sd_legit(monst.dudes[i].monst_start.spec1,
                    monst.dudes[i].monst_start.spec2) == true) &&
          (party.stuff_done[monst.dudes[i].monst_start.spec1]
                           [monst.dudes[i].monst_start.spec2] == 1))
        monst.dudes[i].monst_start.spec_enc_code =
            0; // monster has a life flag and is already dead
      else {
        monst.dudes[i].monst_start.spec_enc_code = 0;
        monst.dudes[i].active = 2;
...
short get_encumberance(short pc_num) {
  ...
  for (i = 0; i < 24; i++)
    if (adven[pc_num].equip[i] == true) {
      ...
      if ((what_val == 1) && (get_ran(1, 0, 130) <
                              hit_chance[adven[pc_num].skills[SKILL_DEFENSE]]))
        what_val--;
      if ((what_val > 1) &&
          (get_ran(1, 0, 70) < hit_chance[adven[pc_num].skills[SKILL_DEFENSE]]))
        what_val--;
      store += what_val;
    }
  return store;
