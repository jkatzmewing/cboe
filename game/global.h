...
typedef struct {
  ...
  // prog_make_ver = scenario "format" version (prog_make_ver[0] == 1 means it's
  // a legacy scenario)
  unsigned char out_width, out_height, difficulty, intro_pic, default_ground;
...
struct party_record_type {
  ...
  unsigned char stuff_done[310][10],
      item_taken[200][8]; // stuff_done[309] is compatibility switches
  short light_level;
  ...
  short m_killed[200];
  unsigned long total_m_killed, total_dam_done, total_xp_gained,
      total_dam_taken;
  char scen_name[256];
