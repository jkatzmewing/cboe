...
void put_spell_info() {
  ...
  if (spell_cost[store_display_mode][pos] > 0)
    sprintf((char *)store_text, "%d/%d", spell_level[pos],
            spell_cost[store_display_mode][pos]);
  else
    sprintf((char *)store_text, "%d/?", spell_level[pos]);
...
void put_item_info() {
  ...
  csp(998, 1, 950);
  if (s_i.graphic_num >= 150) // custom item graphic ?
    csp(998, 1, s_i.graphic_num - 150 + 2000);
  else
    csp(998, 1, 1800 + s_i.graphic_num);
  ...
  if (s_i.isIdent() == false) {
    ...
    cd_set_item_text(998, 12, "???"); // cosmetic change, display ??? in ability
                                      // when the item isn't identified
    return;
  }
  ...
  cd_set_item_num(998, 5,
                  (s_i.charges > 0) ? s_i.value * s_i.charges : s_i.value);
  ...
  } // check if the item ability is to be concealed and print "???" in ability
    // if so
  else {
    GetIndString(desc_str, 23,
                 s_i.ability + 1); // else it display the item ability as usual
    cd_set_item_text(998, 12,
                     desc_str); // note : common items weren't displaying the
                                // "No ability" string. It is restored here.
  }
  if (s_i.charges > 0)
  ...
  case ITEM_TYPE_TWO_HANDED:
    ...
    switch (s_i.type) {
    case ITEM_EDGED:
      sprintf(store_text, "Edged weapon");
      break;
    case ITEM_BASHING:
      sprintf(store_text, "Bashing weapon");
      break;
    case ITEM_POLE:
      sprintf(store_text, "Pole weapon");
      break;
    }
    if (s_i.ability == 0)
...
Boolean display_pc_item_event_filter(short item_hit) {
  ...
  case 14:
    ...
    } while (adven[pc_num].items[item].variety == ITEM_TYPE_NO_ITEM);
...
void put_monst_info() {
  ...
  if (store_m->m_d.spec_skill == MONSTER_INVISIBLE)
    ...
    cd_set_pict(999, 4, 2000 + (store_m->m_d.picture_num % 1000));
  ...
  for (i = 0; i < 3; i++)
    if (store_m->m_d.a[i] > 0) {
      sprintf(store_text, " %dd%d              ", store_m->m_d.a[i] / 100 + 1,
              store_m->m_d.a[i] % 100);
...
Boolean display_monst_event_filter(short item_hit) {
  ...
  case 28:
    ...
    store_m->m_d =
        return_monster_template((unsigned char)on_monst_menu[position]);
    put_monst_info();
    break;
  case 29:
    position++;
...
// short mode; // if 1, full roster, else use monster from storwhich_me_m
{
  ...
  if (mode == 1) {
    ...
    store_m->m_d =
        return_monster_template((unsigned char)on_monst_menu[array_pos]);
  } else {
    hold_m = *which_m;
    store_m = which_m;
  }
...
void pick_race_abil_event_filter(short item_hit) {
  ...
  case 40:
    if (store_trait_mode != 1)
      pc->traits[item_hit - 26] =
          (pc->traits[item_hit - 26] == true) ? false : true;
    display_traits_graphics();
  ...
  default:
    if (item_hit >= 100)
      return;
    if (store_trait_mode != 1)
      pc->traits[item_hit - 7] =
          (pc->traits[item_hit - 7] == true) ? false : true;
    display_traits_graphics();
...
void display_pc_info() {
  ...
  sprintf((char *)to_draw, "%s is carrying %d stones out of %d.",
          adven[pc].name, store, i);
  csit(1019, 69, (char *)to_draw);

  sprintf((char *)str, "%d out of %d.", adven[pc].cur_health,
          adven[pc].max_health);
  csit(1019, 65, (char *)str);
  sprintf((char *)str, "%d out of %d.", adven[pc].cur_sp, adven[pc].max_sp);
  ...
  // Fight bonuses
  for (i = 0; i < 24; i++)
    if (((adven[pc].items[i].variety == ITEM_TYPE_ONE_HANDED) ||
         (adven[pc].items[i].variety == ITEM_TYPE_TWO_HANDED)) &&
  ...
  hit_adj = adven[pc].statAdj(SKILL_DEXTERITY) * 5 -
            (total_encumberance(pc)) * 5 +
            (5 * minmax(-8, 8, (int)adven[pc].status[1]));
  if ((adven[pc].traits[TRAIT_AMBIDEXTROUS] == false) && (weap2 < 24))
    hit_adj -= 25;

  dam_adj = adven[pc].statAdj(SKILL_STRENGTH) +
            minmax(-8, 8, (int)adven[pc].status[STATUS_BLESS_CURSE]);

  if ((skill_item = text_pc_has_abil_equip(pc, ITEM_SKILL)) < 24) {
    hit_adj += 5 * (adven[pc].items[skill_item].ability_strength / 2 + 1);
    dam_adj += adven[pc].items[skill_item].ability_strength / 2;
  }
  if ((skill_item = text_pc_has_abil_equip(pc, ITEM_GIANT_STRENGTH)) < 24) {
    hit_adj += adven[pc].items[skill_item].ability_strength * 2;
    dam_adj += adven[pc].items[skill_item].ability_strength;
  }
  ...
  if (weap1 < 24) {
    if (adven[pc].items[weap1].isIdent() == false)
      ...
      if (hit_adj + 5 * adven[pc].items[weap1].bonus < 0)
        sprintf(to_draw, "Penalty to hit: %%%d",
                hit_adj + 5 * adven[pc].items[weap1].bonus);
      else
        sprintf(to_draw, "Bonus to hit: +%%%d",
                hit_adj + 5 * adven[pc].items[weap1].bonus);
  ...
  if (weap2 < 24) {
    if (adven[pc].items[weap2].isIdent() == false)
      ...
      if (hit_adj + 5 * adven[pc].items[weap2].bonus < 0)
        sprintf(to_draw, "Penalty to hit: %%%d",
                hit_adj + 5 * adven[pc].items[weap2].bonus);
      else
        sprintf(to_draw, "Bonus to hit: +%%%d",
                hit_adj + 5 * adven[pc].items[weap2].bonus);
...
void adventure_notes_event_filter(short item_hit) {
    ...
    if (party.special_notes_str[i][0] > 0) {
      ...
      case 0:
        if (party.special_notes_str[i][0] % 1000 < 160)
          strcpy((char *)place_str,
                 data_store5->scen_strs[party.special_notes_str[i][0] % 1000]);
        else
          strcpy((char *)place_str,
                 scen_strs2[(party.special_notes_str[i][0] % 1000) - 160]);
        break;
      case 1:
        load_outdoors(party.special_notes_str[i][1] % scenario.out_width,
                      party.special_notes_str[i][1] / scenario.out_width, 0, 0,
                      1, party.special_notes_str[i][0] % 1000,
                      (char *)place_str);
        break;
      case 2:
        load_town(party.special_notes_str[i][1], 2,
                  party.special_notes_str[i][0], (char *)place_str);
        break;
      }

      GetIndString(place_str, party.special_notes_str[i][0],
                   party.special_notes_str[i][1]);
      csit(961, 3 + i, (char *)place_str);
      cd_activate_item(961, 9 + i, 1);
    } else
      cd_activate_item(961, 9 + i, 0);
  }
  for (i = store_page_on * 3; i < (store_page_on * 3) + 3; i++) {
    if (party.special_notes_str[i][0] > 0) {
      ...
      case 2:
        load_town(party.special_notes_str[i][1], 2,
                  party.special_notes_str[i][0] % 1000, (char *)place_str);
        break;
      }
...
void put_talk() {
  ...
  if ((personality = party.talk_save[store_page_on].personality) >= 0) {
    ...
    // GetIndString(place_str,120 + ((personality - 1) / 10),((personality - 1)
    // % 10) + 1);
    ...
    if (party.talk_save[store_page_on].str1 >= 1000) {
      if (party.talk_save[store_page_on].str1 >= 3000) {
        if (party.talk_save[store_page_on].str1 - 3000 < 160)
          csit(960, 3,
               data_store5
                   ->scen_strs[party.talk_save[store_page_on].str1 - 3000]);
        else
          csit(960, 3,
               scen_strs2[party.talk_save[store_page_on].str1 - 3000 - 160]);

      } else {
        load_town(party.talk_save[store_page_on].town_num, 2,
                  party.talk_save[store_page_on].str1 - 2000,
                  (char *)place_str);
    ...
    if (party.talk_save[store_page_on].str2 >= 1000) {
      if (party.talk_save[store_page_on].str2 >= 3000) {
        if (party.talk_save[store_page_on].str2 - 3000 < 160)
          csit(960, 5,
               data_store5
                   ->scen_strs[party.talk_save[store_page_on].str2 - 3000]);
        else
          csit(960, 5,
               scen_strs2[party.talk_save[store_page_on].str2 - 3000 - 160]);

      } else {
        load_town(party.talk_save[store_page_on].town_num, 2,
                  party.talk_save[store_page_on].str2 - 2000,
                  (char *)place_str);
...
void give_help(short help1, short help2, short parent_num) {
  ...
  if (help2 == 0)
    display_strings((char *)str1, "", -1, -1, -1, -1, "Instant Help", 57, 724,
                    parent_num);
  else
    display_strings((char *)str1, (char *)str2, -1, -1, -1, -1, "Instant Help",
                    57, 724, parent_num);
}

void put_spec_item_info(short which_i) {
  if (scenario.intro_pic == 100)
    display_strings(data_store5->scen_strs[60 + 1 + which_i * 2], "", -1, -1,
                    -1, -1, data_store5->scen_strs[60 + which_i * 2], 57, 2400,
                    0);
  else
    display_strings(data_store5->scen_strs[60 + 1 + which_i * 2], "", -1, -1,
                    -1, -1, data_store5->scen_strs[60 + which_i * 2], 57,
...
void display_strings_with_nums(short a1, short a2, short b1, short b2,
  ...
  display_strings(str1, str2, -1, -1, -1, -1, title, sound_num, graphic_num,
                  parent_num);
}
