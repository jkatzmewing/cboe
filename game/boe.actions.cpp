...
unsigned char previous_blocked_direction =
    50; // no previous block on initialisation
...
Boolean prime_time() {
  return ((overall_mode < MODE_TALK_TOWN) || (overall_mode == MODE_COMBAT))
             ? true
             : false;
}
...
Boolean handle_action(POINT the_point, WPARAM wparam, LPARAM lparam) {
  ...
  if (((GetAsyncKeyState(VK_LCONTROL) & 32768) == 32768) ||
      ((GetAsyncKeyState(VK_RCONTROL) & 32768) == 32768)) {
    ctrl_key = true;
  }

  for (i = 0; i < 20;
       i++) // emptying the special queue array (events already happened)
    special_queue[i].queued_special = -1;
  queue_position =
      0; // return the position pointer to origin since the array is now empty|
  end_scenario = false;
  ...
  if (button_hit != 12)
    ...
    case 3:
      ...
      if (overall_mode == MODE_OUTDOORS) { // Resting
        ...
        else if ((scenario.ter_types[ter].special >=
                  TER_SPEC_DOES_FIRE_DAMAGE) &&
                 (scenario.ter_types[ter].special <= TER_SPEC_DISEASED_LAND))
    ...
    case 6:
      ...
      if (overall_mode == MODE_COMBAT) {
        ...
        need_reprint = true;
        redraw_terrain();
      } else if ((overall_mode == MODE_FIRING) ||
      ...
      if (overall_mode == MODE_TOWN) {
        ...
        if (which_combat_type == 0) {
          if (hit_end_c_button() == true) {
            ...
            add_string_to_buf("Can't end combat yet.        ");
        } else {
  ...
  if ((PtInRect(&world_screen, the_point)) &&
    ...
    if ((overall_mode == MODE_OUTDOORS) || (overall_mode == MODE_TOWN) ||
      ...
      if ((i == 4) && (j == 4) && (right_button == false)) { // Pausing
        if (overall_mode == MODE_COMBAT) {
          ...
          if (party.in_horse >= 0) {
            if (overall_mode == MODE_OUTDOORS) {
              ...
              party.horses[party.in_horse].horse_loc_in_sec =
                  party.p_loc.toLocal();
              party.horses[party.in_horse].horse_loc = party.p_loc;
              party.horses[party.in_horse].horse_sector.x =
                  party.outdoor_corner.x + party.i_w_c.x;
              party.horses[party.in_horse].horse_sector.y =
        ...
        if ((right_button == false) && (overall_mode == MODE_OUTDOORS)) {
          ...
          if (scenario.ter_types[storage].special ==
              TER_SPEC_TOWN_ENTRANCE) { //// town entry

            if (party.direction == 0)
              find_direction_from = 2;
            else if (party.direction == 4)
              find_direction_from = 0;
            else if (party.direction < 4)
              find_direction_from = 3;
            else
              find_direction_from = 1;

            for (i = 0; i < 8; i++)
              if (same_point(
                      party.loc_in_sec,
                      outdoors[party.i_w_c.x][party.i_w_c.y].exit_locs[i]) ==
                  true) {
                which_t = outdoors[party.i_w_c.x][party.i_w_c.y].exit_dests[i];
                if (which_t >= 0)
                  start_town_mode(
                      outdoors[party.i_w_c.x][party.i_w_c.y].exit_dests[i],
                      find_direction_from);
                if (is_town() == true) {
                  need_redraw = false;
                  i = 8;
    ...
    if ((right_button == true) || (overall_mode == MODE_LOOK_OUTDOORS) ||
        (overall_mode == MODE_LOOK_TOWN) ||
        (overall_mode == MODE_LOOK_COMBAT)) {
      ...
      //			if ((can_see(cur_loc,destination) >= 4) ||
      //((overall_mode != MODE_LOOK_OUTDOORS) && (loc_off_world(destination) ==
      //true)))
      if ((is_combat() == false) && (party_can_see(destination) == 6))
        add_string_to_buf("  Can't see space.         ");
      else if ((is_combat() == true) &&
               (can_see(pc_pos[current_pc], destination, 0) >= 4))
        add_string_to_buf("  Can't see space.         ");
      else {
        ...
        if (scenario.ter_types[ter_looked_at].special == TER_SPEC_IS_A_SIGN) {
            ...
            while (k < 15) {
              if (same_point(destination, c_town.town.sign_locs[k]) == true) {
                ...
                if (adjacent(c_town.town.sign_locs[k], c_town.p_loc) == true)
                  ...
                  add_string_to_buf("  Too far away to read sign.      ");
              }
              k++;
            }
          }
          if (is_out()) {
            ...
            loc_in_sec.y += j - 4;
            while (k < 8) {
              if (same_point(loc_in_sec, outdoors[party.i_w_c.x][party.i_w_c.y]
                                             .sign_locs[k]) == true) {
                need_reprint = true;
                if (adjacent(
                        outdoors[party.i_w_c.x][party.i_w_c.y].sign_locs[k],
                        party.loc_in_sec) == true)
                  do_sign((short)(200 + get_outdoor_num()), k,
                          (short)ter_looked_at);
                else
                  add_string_to_buf("  Too far away to read sign.      ");
              }
    ...
    if (overall_mode == MODE_TALK_TOWN) {
      ...
      if ((can_see(cur_loc, destination, 0) >= 4) ||
          (loc_off_world(destination) == true)) {
        add_string_to_buf("  Can't see space              ");
        need_reprint = true;
      } else {
        ...
        for (i = 0; i < T_M; i++) {
          if (monst_on_space(destination, i) == true) {
            ...
            if (c_town.monst.dudes[i].attitude % 2 == 1) {
              ...
              start_talk_mode(i, c_town.monst.dudes[i].monst_start.personality,
                              c_town.monst.dudes[i].number,
  ...
  if ((overall_mode == MODE_SPELL_TARGET) || (overall_mode == MODE_FIRING) ||
    ...
    if ((PtInRect(&border_rect[0], the_point)) &&
        (center.y > c_town.town.in_town_rect.top) && (center.y > 4)) {
      center.y--;
      need_redraw = true;
    }
    if ((PtInRect(&border_rect[1], the_point)) &&
        (center.x > c_town.town.in_town_rect.left) && (center.x > 4)) {
      center.x--;
      need_redraw = true;
    }
    if ((PtInRect(&border_rect[2], the_point)) &&
        (center.y < c_town.town.in_town_rect.bottom) &&
        (center.y < town_size[town_type] - 5)) {
      center.y++;
      need_redraw = true;
    }
    if ((PtInRect(&border_rect[3], the_point)) &&
        (center.x < c_town.town.in_town_rect.right) &&
  ...
  if (PtInRect(&pc_area_rect, the_point)) {
    ...
    for (i = 0; i < 6; i++)
      for (j = 0; j < 5; j++)
        if ((pc_area_button_active[i][j] > 0) &&
            (PtInRect(&pc_buttons[i][j], point_in_area))) {
          ...
          switch (j) {
          case 0:
            if ((prime_time() == false) && (overall_mode != MODE_SHOPPING) &&
  ...
  if (PtInRect(&item_area_rect, the_point)) {
    ...
    for (i = 0; i < 9; i++)
      if ((item_bottom_button_active[i] > 0) &&
          (PtInRect(&item_screen_button_rects[i], point_in_area))) {
        OffsetRect(&item_screen_button_rects[i], ITEM_WIN_UL_X, ITEM_WIN_UL_Y);
        arrow_button_click(item_screen_button_rects[i]);
        OffsetRect(&item_screen_button_rects[i], -1 * ITEM_WIN_UL_X,
                   -1 * ITEM_WIN_UL_Y);
        switch (i) {
        case 0:
        case 1:
        ...
        case 4:
        case 5:
          if ((prime_time() == false) && (overall_mode != MODE_TALKING) &&
              (overall_mode != MODE_SHOPPING))
            add_string_to_buf("Set active: Finish what you're doing first.");
          else {
            if (!(is_combat())) {
            ...
            if (overall_mode == MODE_SHOPPING) {
              set_up_shop_array();
              draw_shop_graphics(0,
                                 item_screen_button_rects[i]); // rect is dummy
            }
          }
          break;
            ...
            if ((prime_time() == false) && (j < 4) &&
              ...
              case 3: // drop
                if (stat_window == 6) {
                  ...
                  overall_mode =
                      (is_town()) ? MODE_TOWN_DROPPING : MODE_DROPPING;
                }
                break;
              case 4: // info
                if (stat_window == 6)
                  ...
                  display_pc_item(stat_window, item_hit,
                                  adven[stat_window].items[item_hit], 0);
                break;
              case 5: // sell? That this codes was reached indicates that the
                      // item was sellable
                ...
                case 2: // identify item
                  if (party.takeGold(shop_identify_cost, false) == false)
                    ...
                    play_sound(68);
                    sprintf(id_name, "Your item is identified. (%s)",
                            adven[stat_window].items[item_hit].full_name);
                    ASB(id_name);
                ...
                case 6: // enchant item
                  if (party.takeGold(store_selling_values[i], false) == false)
                    ...
                    adven[stat_window].enchantWeapon(
                        item_hit, shop_identify_cost, store_selling_values[i]);
                  }
                  break;
                }
  ...
  if (the_point.x == 1000) {
    ...
    if (overall_mode == MODE_TOWN) {
      ...
      add_string_to_buf("Alchemy: Only in town.");
  }
  ...
  for (i = 0; i < 20; i++)
    if (special_queue[i].queued_special >= 0) {
      ...
      case SPEC_ENTER_TOWN:
        run_special(SPEC_ENTER_TOWN, 2, special_queue[i].queued_special,
                    c_town.p_loc, &s1, &s2, &s3);
        queue_position--; // free the queue slot
        break;
      case SPEC_LEAVE_TOWN:
        run_special(SPEC_LEAVE_TOWN, 2, special_queue[i].queued_special,
                    party.p_loc, &s1, &s2, &s3);
        queue_position--; // free the queue slot
        break;
      default:
        long store_time = party.age;
        party.age = special_queue[i].trigger_time;
        // Call run_special() with the other data from special_queue[i] (which
        // is a pending_special_type struct)
        run_special(special_queue[i].mode, special_queue[i].type,
                    special_queue[i].queued_special, special_queue[i].where,
                    &s1, &s2, &s3);
  ...
  if (did_something == true) {
    ...
    if ((overall_mode >= MODE_COMBAT) && (overall_mode < MODE_TALKING)) {
      ...
      if (!(is_out()) ||
          ((is_out()) && (party.age % 10 ==
                          0))) // no monst move is party outdoors and on horse
        do_monsters();
      if (overall_mode != MODE_OUTDOORS)
        do_monster_turn();
      // Wand monsts
      if ((overall_mode == MODE_OUTDOORS) && (party_toast() == false) &&
          (party.age % 10 == 0)) {
      ...
      if (overall_mode == MODE_TOWN)
        if (get_ran(1, 1,
                    160 - c_town.difficulty + PSD[SDF_LESS_WANDER_ENC] * 200) ==
            2)
          create_wand_monst();
    }
  }
...
Boolean someone_awake() {
  ...
  for (i = 0; i < 6; i++)
    if ((adven[i].isAlive()) && (adven[i].status[STATUS_ASLEEP] <= 0) &&
...
void check_cd_event(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam) {
  ...
  case -1:
    break;
  case 869:
    pick_prefab_scen_event_filter(item_hit);
    break;
  case 947:
    pick_a_scen_event_filter(item_hit);
    break;
  case 958:
    tip_of_day_event_filter(item_hit);
    break;
  case 960:
    talk_notes_event_filter(item_hit);
    break;
  case 961:
    adventure_notes_event_filter(item_hit);
    break;
  case 970:
  case 971:
  case 972:
  case 973:
    display_strings_event_filter(item_hit);
    break;
  case 987:
    display_item_event_filter(item_hit);
    break;
  case 988:
    pick_trapped_monst_event_filter(item_hit);
    break;
  case 989:
    edit_party_event_filter(item_hit);
    break;
  case 991:
    display_pc_event_filter(item_hit);
    break;
  case 996:
    display_alchemy_event_filter(item_hit);
    break;
  case 997:
    display_help_event_filter(item_hit);
    break;
  case 998:
    display_pc_item_event_filter(item_hit);
    break;
  case 999:
    display_monst_event_filter(item_hit);
    break;
  case 1010:
    spend_xp_event_filter(item_hit);
    break;
  case 1012:
    get_num_of_items_event_filter(item_hit);
    break;
  case 1013:
    pick_race_abil_event_filter(item_hit);
    break;
  case 1014:
  ...
  case 1017:
  case 873:
    get_text_response_event_filter(item_hit);
    break;
  case 1018:
    select_pc_event_filter(item_hit);
    break;
  case 1019:
    give_pc_info_event_filter(item_hit);
    break;
  case 1047:
    alch_choice_event_filter(item_hit);
    break;
  case 1050:
    pc_graphic_event_filter(item_hit);
    break;
  case 1051:
    pc_name_event_filter();
    break;
  case 1096:
    display_spells_event_filter(item_hit);
    break;
  case 1097:
    display_skills_event_filter(item_hit);
    break;
  case 1098:
    pick_spell_event_filter(item_hit);
    break;
  case 1099:
    prefs_event_filter(item_hit);
    break;
  case 1100:
    compatibility_event_filter(item_hit);
    break;
  default:
    fancy_choice_dialog_event_filter(item_hit);
    break;
  }
}
...
void handle_menu_spell(short spell_picked, short spell_type) {
  ...
  if ((spell_type == 0) && (mage_need_select[spell_picked] > 0)) {
    if ((store_spell_target = char_select_pc(2 - mage_need_select[spell_picked],
                                             0, "Cast spell on who?")) == 6)
      return;
  } else {
    if ((spell_type == 1) && (priest_need_select[spell_picked] > 0))
      if ((store_spell_target =
               char_select_pc(2 - priest_need_select[spell_picked], 0,
                              "Cast spell on who?")) == 6)
        return;
  }
...
void initiate_outdoor_combat(short i) {
  ...
  // Is combat too easy?
  if ((adven.getTotalLevel() > ((out_enc_lev_tot(i) * 5) / 3)) &&
      (out_enc_lev_tot(i) < 200) &&
  ...
  start_outdoor_combat(party.out_c[i], out[party.p_loc.x][party.p_loc.y],
                       party.p_loc.countWalls());
...
{
  if ((overall_mode < MODE_TALKING) || (overall_mode >= 30))
    draw_terrain(0);
}
...
Boolean handle_syskeystroke(WPARAM wParam, LPARAM, short *handled) {
  ...
  POINT terrain_click[10] = {{150, 185}, {120, 215}, {150, 215}, {180, 215},
                             {120, 185}, {150, 185}, {180, 185}, {120, 155},
                             {150, 155}, {180, 135}};
  POINT pass_point;
  Boolean are_done = false;
  short i;
  ...
  if ((wParam == VK_HOME) || (wParam == VK_PRIOR)) { // go northwest or
                                                     // northeast
    *handled = 1;
...
Boolean handle_keystroke(WPARAM wParam, LPARAM lParam) {
  ...
  if ((overall_mode != MODE_TALKING) && (overall_mode != MODE_SHOPPING)) {
    ...
    for (k = 0; k < (short)LOWORD(lParam); k++)
      ...
      case ' ':
        if (overall_mode ==
            MODE_FANCY_TARGET) { // cast multi-target spell, set # targets to 0
                                 // so that space clicked doesn't matter
          num_targets_left = 0;
      ...
      case 'B': // Leave town
        if (in_scen_debug) {
          ...
          overall_mode = MODE_OUTDOORS;
          position_party(party.outdoor_corner.x, party.outdoor_corner.y,
      ...
      case 'C': // Clean up
        ...
        // Can be moved out of this function to allow the character to switch
        // debug mode during dialogues or shopping.
      ...
      case 'K':
        if (in_scen_debug) {
          for (i = 0; i < T_M; i++) {
            if ((is_combat()) && (c_town.monst.dudes[i].active > 0) &&
      ...
      case 'O': // Location
        ...
        if (is_combat()) {
          ...
          sprintf(c_line, "Debug:  You're in combat at x %d,  y %d.", loc.x,
                  loc.y);
        }
      ...
      case 'I': // debug info
                ...
                // sprintf(buf,"Party Age : %d, Ctown ploc : x = %d, y = %d",
                // party.age, center.x, center.y);
        //           sprintf(buf,"Town chop time : %d, chop key :
        //           %d",c_town.town.town_chop_time, c_town.town.town_chop_key);
        /*sprintf(buf,"Scenario prog version %d %d %d",
        scenario.prog_make_ver[0], scenario.prog_make_ver[1],
        scenario.prog_make_ver[2]); give_error(buf,"",0);*/
        break;
      case 'Q': // Magic map
        if (in_scen_debug) {
          if (overall_mode == MODE_OUTDOORS) {
      ...
      case 'R': // Return to Start
        ...
        break;
      case 'S':              // Set a SDF
        if (in_scen_debug) { // and leave alone the 300+ SDF (game
                             // options/mechanics)
          char response[256] = "";
          display_strings("Enter Stuff Done Flag Part A (between 0 and 299)",
                          "", 0, 0, 0, 0, "Which SDFa ?", -1, 130, 0);
          get_text_response(873, response, 0);
          i = atoi(response);
          if (i >= 0 && i < 300) {
            display_strings("Enter Stuff Done Flag Part B (between 0 and 9)",
                            "", 0, 0, 0, 0, "Which SDFb ?", -1, 130, 0);
            get_text_response(873, response, 0);
            j = atoi(response);
            if (j >= 0 && j < 10) {
              display_strings(
                  "Enter Stuff Done Flag Part Value (outrageous values are a "
                  "sure way to crash, or at least broke, the game)|Entering -1 "
                  "show the value of the specified SDF.",
                  "", 0, 0, 0, 0, "Which value ?", -1, 130, 0);
              get_text_response(873, response, 0);
              x = atoi(response);
              if (x == -1) {
                sprintf(response, "SDF[%d][%d] = %d", i, j,
                        party.stuff_done[i][j]);
      ...
      case 'T': // Enter a Town
        if (in_scen_debug) {
          ...
          char response[256] = "";
          sprintf(response, "Enter Town Number (between 0 and %d)",
      ...
      case 'b':
      case 'u':
      case 'L': // End combat, shoot and only active character <= why did you
                // remove 'b' ?
        if (overall_mode == MODE_TOWN) {
      ...
      case 'w':
        ...
        if ((chr == 'm') &&
            ((overall_mode == MODE_SPELL_TARGET) ||
             (overall_mode == MODE_FANCY_TARGET))) // cancel spell
          j = 0;
        if ((chr == 'p') &&
            ((overall_mode == MODE_SPELL_TARGET) ||
             (overall_mode == MODE_FANCY_TARGET))) // cancel spell
...
void increase_age() {
  ...
  if (party.stuff_done[SDF_FLYING] == 1) {
    if (scenario.ter_types[out[party.p_loc.x][party.p_loc.y]].blockage > 2) {
      ...
      add_string_to_buf("  You land safely.                  ");
    reset_text_bar();
  }
  ...
  // Protection, etc.
  for (i = 0; i < NUM_OF_PCS;
       i++) { // Process some status things, and check if stats updated

    if ((adven[i].status[STATUS_INVULNERABLE] == 1) ||
        (adven[i].status[5] == 1) || (adven[i].status[8] == 1) ||
        (adven[i].status[STATUS_ASLEEP] == 1) ||
        (adven[i].status[STATUS_PARALYZED] == 1))
      update_stat = true;
  ...
  for (i = 0; i < 6; i++) // Poison
    if (adven[i].status[STATUS_POISON] > 0) {
      i = 6;
      if (((overall_mode == MODE_OUTDOORS) && (party.age % 50 == 0)) ||
          ((overall_mode == MODE_TOWN) && (party.age % 20 == 0))) {
        update_stat = true;
        do_poison();
      }
    }
  for (i = 0; i < 6; i++) // Disease
    if (adven[i].status[STATUS_DISEASE] > 0) {
      i = 6;
      if (((overall_mode == MODE_OUTDOORS) && (party.age % 100 == 0)) ||
          ((overall_mode == MODE_TOWN) && (party.age % 25 == 0))) {
        update_stat = true;
        handle_disease();
      }
    }
  ...
  for (i = 0; i < 6; i++)
    if (adven[i].isAlive()) {
      if ((adven[i].traits[TRAIT_RECUPERATION] > 0) &&
          (get_ran(1, 0, 10) == 1) &&
          (adven[i].cur_health < adven[i].max_health)) {
        adven[i].heal(2);
        update_stat = true;
      }
      if ((adven[i].traits[TRAIT_CHRONIC_DISEASE] > 0) &&
          (get_ran(1, 0, 110) == 1)) {
        adven[i].disease(4);
        update_stat = true;
      }
...
void start_new_game() {
  ...
  for (i = 0; i < 6; i++)
    if (adven[i].isAlive()) {
      ...
      adven[i].max_sp += adven[i].skills[SKILL_MAGE_SPELLS] * 3 +
                         adven[i].skills[SKILL_PRIEST_SPELLS] * 3;
      adven[i].cur_sp = adven[i].max_sp;
    }
  save_file(1);
...
location get_cur_direction(POINT the_point) {
  ...
  // This is a kludgy adjustment to adjust for the screen shifting between Exile
  // I & II
  y += 5;
  x += 5;

  if ((x < 135) & (y >= ((x * 34) / 10) - 293) &
      (y <= (-1 * ((x * 34) / 10) + 663)))
    store_dir.x--;
  if ((x > 163) & (y <= ((x * 34) / 10) - 350) &
      (y >= (-1 * ((x * 34) / 10) + 721)))
    store_dir.x++;
...
Boolean outd_move_party(location destination, Boolean forced) {
  ...
  if (in_scen_debug && ghost_mode)
    forced = true;
  /*if (spec_num == 50)		forced = true;*/ // if it's the number 50
                                                 // special node, then force
                                                 // movement...
  ...
  if ((keep_going == true) && (overall_mode == MODE_OUTDOORS)) {
    ...
    if ((destination.x < 6) && (party.outdoor_corner.x > 0))
      shift_universe_left();
    if ((destination.x > 90) &&
        (party.outdoor_corner.x < scenario.out_width - 1))
      shift_universe_right();
    if ((destination.y < 6) && (party.outdoor_corner.y > 0))
      shift_universe_up();
    else if ((destination.y > 90) &&
             (party.outdoor_corner.y < scenario.out_height - 1))
      shift_universe_down();
    ...
    if (((real_dest.x > 92) &&
         (party.outdoor_corner.x >= scenario.out_width - 2)) ||
        ((real_dest.x > 44) &&
         (party.outdoor_corner.x >= scenario.out_width - 1))) {
      ASB("You've reached the world's edge.");
      return false;
    ...
    if ((real_dest.y < 4) && (party.outdoor_corner.y <= 0)) {
      ASB("You've reached the world's edge.");
      return false;
    } else if (((real_dest.y > 92) &&
                (party.outdoor_corner.y >= scenario.out_height - 2)) ||
               ((real_dest.y > 44) &&
                (party.outdoor_corner.y >= scenario.out_height - 1))) {
      ASB("You've reached the world's edge.");
      return false;
    ...
    if (party.in_boat >= 0) {
      if ((outd_is_blocked(real_dest) ==
           false) //&& (outd_is_special(real_dest) == false)
          // not in towns
          && ((scenario.ter_types[ter].boat_over == false) ||
              ((real_dest.x != party.p_loc.x) &&
               (real_dest.y != party.p_loc.y))) &&
          (scenario.ter_types[ter].special != TER_SPEC_TOWN_ENTRANCE)) {
        add_string_to_buf("You leave the boat.");
        party.in_boat = -1;
      } else if (((real_dest.x != party.p_loc.x) &&
                  (real_dest.y != party.p_loc.y)) ||
        ...
        return false;
               ...
               (scenario.ter_types[ter].special != TER_SPEC_TOWN_ENTRANCE) &&
               (ter != 75) &&
               (ter != 76)) { // 75-76=> lava is hardcoded... to change
    ...
    if (((boat_num = out_boat_there(real_dest)) < 30) && (party.in_boat < 0) &&
        (party.in_horse < 0)) {
      if (party.isFlying()) {
      ...
      if ((store_corner.x != party.outdoor_corner.x) ||
          (store_corner.y != party.outdoor_corner.y) ||
          (store_iwc.x != party.i_w_c.x) || (store_iwc.y != party.i_w_c.y))
        clear_map();

      return true;
    } else if (((horse_num = out_horse_there(real_dest)) < 30) &&
               (party.in_boat < 0) && (party.in_horse < 0)) {
      if (party.isFlying()) {
      ...
      if (party.in_horse >= 0) {
        if ((scenario.ter_types[ter].special >= TER_SPEC_DOES_FIRE_DAMAGE) &&
            (scenario.ter_types[ter].special <= TER_SPEC_DOES_MAGIC_DAMAGE)) {
      ...
      if (party.in_boat >= 0) {
        // Waterfall!!!
        while (
            scenario.ter_types[out[party.p_loc.x][party.p_loc.y + 1]].special ==
            TER_SPEC_WATERFALL) {
        ...
        party.boats[party.in_boat].boat_sector.x =
            party.outdoor_corner.x + party.i_w_c.x;
        party.boats[party.in_boat].boat_sector.y =
            party.outdoor_corner.y + party.i_w_c.y;
      }
      if (party.in_horse >= 0) {
        party.horses[party.in_horse].which_town = 200;
      ...
      if (set_direction(party.p_loc, destination) ==
          previous_blocked_direction) {
        times_blocked++;
        buffer_ptr =
            ((buf_pointer - 1) == -1) ? (TEXT_BUF_LEN - 1) : buf_pointer - 1;
        sprintf(text_buffer[buffer_ptr].line, "Blocked: %s (x%d)",
                dir_string[set_direction(party.p_loc, destination)],
                times_blocked);
        string_added = true;
        print_buf();
      } else {
...
Boolean town_move_party(location destination, short forced) {
  ...
  if (check_f == true)
    forced = true;
  /*if (spec_num == 50) forced = true;*/ // if it's the number 50 special node,
                                         // then force movement...
  ...
  if (keep_going == true) {
    if (party.in_boat >= 0) {
      if ((is_blocked(destination) == false) &&
          (is_special(destination) == false)
          // If to bridge, exit if heading diagonal, keep going is head horiz or
          // vert
          && ((scenario.ter_types[ter].boat_over == false) ||
              ((destination.x != c_town.p_loc.x) &&
               (destination.y != c_town.p_loc.y)))) {
        add_string_to_buf("You leave the boat.             ");
        party.in_boat = -1;
      } else if ((destination.x != c_town.p_loc.x) &&
                 (destination.y != c_town.p_loc.y))
        return false;
      // Crossing bridge: land or go through
      else if ((is_blocked(destination) == false) &&
               (scenario.ter_types[ter].boat_over == true) && ter != 75 &&
               ter != 76) {
        if ((fancy_choice_dialog(1086, 0)) == 1)
          forced = true;
        else if (is_blocked(destination) == false) {
    ...
    if (((boat_there = town_boat_there(destination)) < 30) &&
        (party.in_boat < 0)) {
      ...
      return true;
    } else if (((horse_there = town_horse_there(destination)) < 30) &&
               (party.in_horse < 0)) {
      ...
      if (party.in_boat >=
          0) { // Watchout, party.p_loc is party outdoor location !!! Town
               // location is c_town.p_loc ...
        ...
        if (party.stuff_done[SDF_COMPATIBILITY_WORKING_TOWN_WATERFALL] > 0) {
          ...
          // Waterfall!!!
          while (scenario
                     .ter_types[t_d.terrain[c_town.p_loc.x][c_town.p_loc.y + 1]]
      ...
      if (destination.isDoor()) {
        sprintf(create_line, "Door locked: %s               ",
                dir_string[set_direction(c_town.p_loc, destination)]);
        add_string_to_buf(create_line);
      } else {
        if (set_direction(c_town.p_loc, destination) ==
            previous_blocked_direction) {
          times_blocked++;
          buffer_ptr =
              ((buf_pointer - 1) == -1) ? (TEXT_BUF_LEN - 1) : buf_pointer - 1;
...
short get_outdoor_num() {
  return (scenario.out_width * (party.outdoor_corner.y + party.i_w_c.y) +
          party.outdoor_corner.x + party.i_w_c.x);
}
...
void set_up_lights() {
  ...
  short max_dim[3] = {64, 48,
                      32}; // Town sizes from Scenario Editor blscened.cpp:82
  ...
  for (i = 0; i < max_dim[town_type]; i++)
    for (j = 0; j < max_dim[town_type]; j++) {
      ...
      if (rad > 0) {
        for (where.x = max(0, i - rad);
             where.x < min(max_dim[town_type], i + rad + 1); where.x++)
          for (where.y = max(0, j - rad);
               where.y < min(max_dim[town_type], j + rad + 1); where.y++)
            if ((where_lit[where.x][where.y] == 0) && (dist(where, l) <= rad) &&
                (can_see(l, where, 0) < 5))
  ...
  for (where.x = 0; where.x < max_dim[town_type]; where.x++)
    for (where.y = 0; where.y < max_dim[town_type]; where.y++) {
      if (where_lit[where.x][where.y] > 0) {
        t_d.lighting[where.x / 8][where.y] =
            t_d.lighting[where.x / 8][where.y] | (char)(s_pow(2, where.x % 8));
      }
    }
}
