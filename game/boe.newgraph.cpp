...
void apply_unseen_mask() {
  RECT base_rect = {8, 10, 43, 53} /*{9,9,43,53}*/, to_rect,
       big_to = {13, 13, 265, 337}; /**/
  short i, j, k, l;
  ...
  for (i = 0; i < 11; i++)
    for (j = 0; j < 11; j++)
      if (unexplored_area[i + 1][j + 1] == 1) {
        ...
        Rectangle(hdc, to_rect.left, to_rect.top, to_rect.right,
                  to_rect.bottom);
        // PaintRect(&to_rect);
        for (k = i - 2; k < i + 1; k++)
          for (l = j - 2; l < j + 1; l++)
...
void apply_light_mask() {
  ...
  if (same_mask == true) {
    ...
    SelectObject(hdc, store_bmp);
    if (!DeleteDC(hdc))
      DebugQuit("Cannot release DC 2");
    return;
  }
  SetRectRgn(dark_mask_region, big_to.left, big_to.top, big_to.right,
             big_to.bottom);
  for (i = 0; i < 13; i++)
    for (j = 0; j < 13; j++)
  ...
  for (i = 1; i < 12; i++)
    for (j = 1; j < 12; j++) {
      if (light_area[i][j] == 2) {
        ...
        OffsetRgn(oval_region, -13 + -1 * (28 * (i - 3)),
                  -13 + -1 * (36 * (j - 3)));
      }
      if (light_area[i][j] == 3) {
        ...
        if (light_area[i + 1][j] == 3)
          light_area[i + 1][j] = 0;
        if (light_area[i + 1][j + 1] == 3)
          light_area[i + 1][j + 1] = 0;
        if (light_area[i][j + 1] == 3)
          light_area[i][j + 1] = 0;
      }
    }
...
void end_missile_anim() { boom_anim_active = false; }

void add_missile(location dest, short missile_type, short path_type,
                 short x_adj, short y_adj) {
  short i;
  ...
  for (i = 0; i < 30; i++)
    if ((store_missiles[i].missile_type >= 0) &&
        (same_point(dest, store_missiles[i].dest) == true))
      return;
  for (i = 0; i < 30; i++)
    if (store_missiles[i].missile_type < 0) {
      have_missile = true;
...
void run_a_missile(location from, location fire_to, short miss_type, short path,
                   short sound_num, short x_adj, short y_adj, short len) {
  start_missile_anim();
...
void add_explosion(location dest, short val_to_place, short place_type,
                   short boom_type, short x_adj, short y_adj) {
  short i;
  ...
  for (i = 0; i < 30; i++)
    if ((store_booms[i].boom_type >= 0) &&
        (same_point(dest, store_booms[i].dest) == true) && (place_type == 0)) {
      if (val_to_place > 0)
...
void do_missile_anim(short num_steps, location missile_origin,
                     short sound_num) {
  RECT temp_rect, missile_origin_base = {1, 1, 17, 17}, active_area_rect,
                  to_rect, from_rect;
  ...
  temp_gworld = CreateCompatibleBitmap(main_dc, ter_scrn_rect.right,
                                       ter_scrn_rect.bottom);
  ...
  for (i = 0; i < 30; i++) {
    ...
    if ((store_missiles[i].missile_type >= 0) &&
        (same_point(missile_origin, store_missiles[i].dest) == true))
      store_missiles[i].missile_type = -1;
  }
  screen_ul.x = center.x - 4;
  screen_ul.y = center.y - 4;
  start_point.x = 13 + 14 + 28 * (short)(missile_origin.x - screen_ul.x);
  start_point.y = 13 + 18 + 36 * (short)(missile_origin.y - screen_ul.y);
  for (i = 0; i < 30; i++)
    if (store_missiles[i].missile_type >= 0) {
      finish_point[i].x = 1 + 13 + 14 + store_missiles[i].x_adj +
                          28 * (short)(store_missiles[i].dest.x - screen_ul.x);
      finish_point[i].y = 1 + 13 + 18 + store_missiles[i].y_adj +
                          36 * (short)(store_missiles[i].dest.y - screen_ul.y);
      ...
      if (store_missiles[i].missile_type < 7) {
        ...
        OffsetRect(&missile_origin_rect[i], 18 * store_missile_dir,
                   18 * store_missiles[i].missile_type);
      } else {
        missile_origin_rect[i] = missile_origin_base;
        OffsetRect(&missile_origin_rect[i], 0,
                   18 * store_missiles[i].missile_type);
      }
      ...
      missile_place_rect[i].top = missile_place_rect[i].left =
          missile_place_rect[i].bottom = missile_place_rect[i].right = 0;
  ...
  for (t = 0; t < num_steps; t++) {
    for (i = 0; i < 30; i++)
      if (store_missiles[i].missile_type >= 0) {
        ...
        rect_draw_some_item(missiles_gworld, from_rect, temp_gworld, temp_rect,
                            1, 0);
      }
    ...
    for (i = 0; i < 30; i++)
      if (store_missiles[i].missile_type >= 0) {
        ...
        OffsetRect(&to_rect, current_terrain_ul.x, current_terrain_ul.y);
        rect_draw_some_item(temp_gworld, missile_place_rect[i], temp_gworld,
                            to_rect, 0, 1);
      }
    if ((PSD[SDF_GAME_SPEED] == 3) ||
        ((PSD[SDF_GAME_SPEED] == 1) && (t % 4 == 0)) ||
...
short get_missile_direction(POINT origin_point, POINT the_point) {
  location store_dir;
  short dir = 0;
  // To reuse legacy code, will renormalize the_point, which is missile
  // destination, so that origin_point is moved to (149,185) and the_point is
  // moved in proportion
  ...
  case 0:
    ...
    break;
  case -1:
    dir = (store_dir.x == -1) ? 7 : store_dir.x;
    break;
  case 1:
    dir = 4 - store_dir.x;
    break;
...
// special_draw - 0 normal 1 - first half 2 - second half
{
  ...
  RECT ter_scrn_rect = {0, 0, 279, 351};
  HDC hdc;
  COLORREF colors[5] = {RGB(0, 0, 0), RGB(255, 0, 0), RGB(128, 0, 0),
                        RGB(0, 160, 0), RGB(255, 255, 255)};
  HBITMAP store_bmp;
  long t1, t2;
  ...
  for (i = 0; i < 30; i++)
    if ((store_booms[i].boom_type >= 0) && (special_draw < 2)) {
      ...
      OffsetRect(&explode_place_rect[i],
                 13 + 28 * (store_booms[i].dest.x - screen_ul.x) +
                     store_booms[i].x_adj,
                 13 + 36 * (store_booms[i].dest.y - screen_ul.y) +
                     store_booms[i].y_adj);
      ...
      explode_place_rect[i].top = explode_place_rect[i].left =
          explode_place_rect[i].bottom = explode_place_rect[i].right = 0;

  if (special_draw < 2)
    play_sound(boom_type_sound[cur_boom_type]);

  // Now, at last, do explosion
  for (t = (special_draw == 2) ? 6 : 0; t < ((special_draw == 1) ? 6 : 11);
       t++) { // t goes up to 10 to make sure screen gets cleaned up
    ...
    for (i = 0; i < 30; i++)
      if (store_booms[i].boom_type >= 0) {
        if ((t + store_booms[i].offset >= 0) &&
            (t + store_booms[i].offset <= 7)) {
          from_rect = base_rect;
          OffsetRect(&from_rect, 28 * (t + store_booms[i].offset),
                     144 + 36 * (store_booms[i].boom_type));
          rect_draw_some_item(fields_gworld, from_rect, temp_gworld,
                              explode_place_rect[i], 1, 0);
    ...
    for (i = 0; i < 30; i++)
      if (store_booms[i].boom_type >= 0) {
        ...
        OffsetRect(&to_rect, current_terrain_ul.x, current_terrain_ul.y);
        rect_draw_some_item(temp_gworld, explode_place_rect[i], temp_gworld,
                            to_rect, 0, 1);
      }
...
// mode 1 - drawing dark for button press
{
  ...
  char const *cost_strs[] = {"Extremely Cheap",   "Very Reasonable",
                             "Pretty Average",    "Somewhat Pricey",
                             "Expensive",         "Exorbitant",
                             "Utterly Ridiculous"};
  item_record_type base_item;
  HDC hdc;
  COLORREF colors[7] = {RGB(0, 0, 0),      RGB(255, 0, 130), RGB(128, 0, 70),
                        RGB(0, 0, 100),    RGB(0, 0, 220),   RGB(0, 220, 0),
                        RGB(255, 110, 255)};
  HBITMAP store_bmp;
  HBRUSH old_brush;
  ...
  if (draw_mode > 0) {
    IntersectClipRect(hdc, clip_area_rect.left, clip_area_rect.top,
                      clip_area_rect.right, clip_area_rect.bottom);
  }
  ...
  Rectangle(hdc, area_rect.left, area_rect.top, area_rect.right,
            area_rect.bottom);
  SelectObject(hdc, old_brush);
  ...
  if (draw_mode > 0) {
    ...
    paint_pattern(talk_gworld, 0, area_rect, 3);

  SelectObject(hdc, talk_gworld);

  old_brush = (HBRUSH)SelectObject(hdc, GetStockObject(NULL_BRUSH));
  Rectangle(hdc, shop_frame.left, shop_frame.top, shop_frame.right,
            shop_frame.bottom);
  SelectObject(hdc, old_brush);
  ...
  case SHOP_HEALER:
    sprintf(cur_name, "Healing for %s.", adven[current_pc].name);
    break;
  case SHOP_MAGE_SPELLS:
    sprintf(cur_name, "Mage Spells for %s.", adven[current_pc].name);
    break;
  case SHOP_PRIEST_SPELLS:
    sprintf(cur_name, "Priest Spells for %s.", adven[current_pc].name);
    break;
  case SHOP_ALCHEMY:
    sprintf(cur_name, "Buying Alchemy.");
    break;
  case SHOP_FOOD:
    sprintf(cur_name, "Buying Food.");
    break;
  default:
    sprintf(cur_name, "Shopping for %s.", adven[current_pc].name);
    break;
  }
  ...
  rect_draw_some_item(dlg_buttons_gworld, help_from, talk_gworld,
                      talk_help_rect, 0, 0);
  rect_draw_some_item(dlg_buttons_gworld, done_from, talk_gworld,
                      shop_done_rect, 0, 0);
  SelectObject(hdc, talk_gworld);
  if (draw_mode == 0)
    SetTextColor(hdc, colors[0]);
  ...
  for (i = 0; i < 8; i++) {
    ...
    case 4:
      ...
      draw_dialog_graphic((HWND)talk_gworld, shopping_rects[i][2],
                          1800 + base_item.graphic_num, false, 1);
    ...
    case 5:
      ...
      draw_dialog_graphic((HWND)talk_gworld, shopping_rects[i][2], 1853, false,
                          1); //// all graphic nums
      strcpy(cur_name, base_item.full_name);
    ...
    case 7:
      what_chosen -= 700;
      draw_dialog_graphic((HWND)talk_gworld, shopping_rects[i][2], 1879, false,
                          1);
    ...
    default:
      ...
      base_item =
          party.magic_store_items[what_magic_shop][what_magic_shop_item];
      base_item.item_properties = base_item.item_properties | 1;
      draw_dialog_graphic((HWND)talk_gworld, shopping_rects[i][2],
                          1800 + base_item.graphic_num, false, 1);
      strcpy(cur_name, base_item.full_name);
    ...
    shopping_rects[i][6].bottom =
        shopping_rects[i][6].top + (from_rect.bottom - from_rect.top);
    if ((store_shop_type != SHOP_HEALER) && (store_shop_type != SHOP_FOOD))
      rect_draw_some_item(mixed_gworld, item_info_from, talk_gworld,
                          shopping_rects[i][6], 1, 0);
    SelectObject(hdc, talk_gworld);
  ...
  char_win_draw_string(hdc, bottom_help_rects[1],
                       "Click on item name (or type 'a'-'h') to buy.", 0, 12);
  char_win_draw_string(hdc, bottom_help_rects[2],
                       "Hit done button (or Esc.) to quit.", 0, 12);
  if ((store_shop_type != SHOP_HEALER) && (store_shop_type != SHOP_FOOD))
    char_win_draw_string(hdc, bottom_help_rects[3],
                         "'I' button brings up description.", 0, 12);

  SelectObject(hdc, store_bmp);
  if (!DeleteDC(hdc))
    DebugQuit("Cannot release DC 8");
...
void click_talk_rect(char const *str_to_place, char const *str_to_place2,
                     RECT c_rect) {
...
item_record_type store_mage_spells(short which_s) {
  item_record_type spell = {21, 0, 0, 0, 0, 0,          0,  0,  53, 0, 0, 0,
                            0,  0, 0, 0, 0, location(), "", "", 0,  0, 0, 0};
...
item_record_type store_alchemy(short which_s) {
  item_record_type spell = {21, 0, 0, 0, 0, 0,          0,  0,  53, 0, 0, 0,
                            0,  0, 0, 0, 0, location(), "", "", 0,  0, 0, 0};
  short val[20] = {50,  75,  30,  130, 100, 150,  200,   200,  300,  250,
                   300, 500, 600, 750, 700, 1000, 10000, 5000, 7000, 12000};
  char str[256];
...
void get_item_interesting_string(item_record_type item, char *message) {
  ...
  case ITEM_TYPE_MISSILE_NO_AMMO: ////
    if (item.bonus != 0)
      ...
      sprintf(message, "Damage: 1-%d.", item.item_level);
    break;
  ...
  case ITEM_TYPE_BOOTS:
    sprintf(message, "Blocks %d-%d damage.",
            item.item_level + (item.protection > 0) ? 1 : 0,
  ...
  case ITEM_TYPE_WEAPON_POISON:
    sprintf(message, "Poison: Does %d-%d damage.", item.item_level,
            item.item_level * 6);
    break;
  default:
    sprintf(message, "");
...
void place_talk_str(char const *str_to_place, char const *str_to_place2,
...
// color 0 - regular  1 - darker
{
  ...
  short text_len[257], current_rect, store_last_word_break = 0,
                                     start_of_last_kept_word = -1;
  short last_line_break = 0, last_word_break = 0, on_what_line = 0,
        last_stored_word_break = 0;
  Boolean force_skip = false;
  short face_to_draw;

  HDC hdc;
  COLORREF colors[7] = {RGB(0, 0, 0),   RGB(0, 0, 204), RGB(0, 0, 102),
                        RGB(0, 0, 100), RGB(0, 0, 220), RGB(51, 153, 51),
                        RGB(0, 204, 0)};
  HBITMAP store_bmp;
  HBRUSH old_brush;
  HFONT store_font;
  ...
  if (c_rect.right > 0) {
    IntersectClipRect(hdc, c_rect.left, c_rect.top, c_rect.right,
                      c_rect.bottom);
  }
  ...
  if ((color == 0) && (c_rect.right == 0)) {
    ...
    if (store_talk_face_pic >= 1000) {
      draw_dialog_graphic((HWND)talk_gworld, face_rect,
                          2400 + store_talk_face_pic - 1000, false, 1);
    } else {
      ...
      if (face_to_draw <= 0)
        ...
        draw_dialog_graphic((HWND)talk_gworld, face_rect, 1000 + face_to_draw,
                            false, 1);
    }
  }
  SelectObject(hdc, talk_gworld);
  ...
  for (i = 0; i < 9; i++)
    if ((talk_end_forced == false) || (i == 6) || (i == 5)) {
      ...
      char_win_draw_string(hdc, preset_words[i].word_rect, preset_words[i].word,
                           2, 18);
      OffsetRect(&preset_words[i].word_rect, 0, 2);
    }
  ...
  MoveTo(dest_rect.left + 1,
         dest_rect.top + 1 + line_height * on_what_line + 9);

  for (i = 0; i < str_len; i++) {
    if (((str[i] != 39) && ((str[i] < 65) || (str[i] > 122)) &&
         ((str[i] < 48) || (str[i] > 57))) &&
        (color == 0)) { // New word, so set up a rect
      if (((i - store_last_word_break >= 4) || (i >= str_len - 1)) &&
          (i - last_stored_word_break >= 4) && (talk_end_forced == false)) {
        store_words[current_rect].word_rect.left =
            dest_rect.left +
            (text_len[store_last_word_break] - text_len[last_line_break]) - 2;
        store_words[current_rect].word_rect.right =
            dest_rect.left + (text_len[i + 1] - text_len[last_line_break]) - 1;
        store_words[current_rect].word_rect.top =
            dest_rect.top + 1 + line_height * on_what_line - 5;
        store_words[current_rect].word_rect.bottom =
            dest_rect.top + 1 + line_height * on_what_line + 13;

        if ((str[store_last_word_break] < 48) ||
            (str[store_last_word_break] == 96) ||
            (str[store_last_word_break] > 122) ||
            ((str[store_last_word_break] >= 58) &&
             (str[store_last_word_break] <= 64)))
        ...
        for (j = 0; j < 4; j++)
          if ((store_words[current_rect].word[j] >= 65) &&
              (store_words[current_rect].word[j] <= 90))
            store_words[current_rect].word[j] += 32;
        if (scan_for_response(store_words[current_rect].word) < 0) {
          store_words[current_rect].word_rect.left =
              store_words[current_rect].word_rect.right = 0;
        } else {
    ...
    if (((text_len[i] - text_len[last_line_break] >
          (dest_rect.right - dest_rect.left - 6)) &&
         (last_word_break > last_line_break)) ||
        (str[i] == '|') || (i == str_len - 1)) {
      if (str[i] == '|') {
        str[i] = ' ';
        force_skip = true;
      ...
      sprintf((char *)str_to_draw,
              "                                                         ");
      strncpy((char *)str_to_draw, (char *)str + last_line_break,
              (size_t)(last_word_break - last_line_break - 1));
      ...
      if ((start_of_last_kept_word >= last_line_break) && (current_rect > 0)) {
        OffsetRect(&store_words[current_rect - 1].word_rect,
                   5 + -1 * store_words[current_rect - 1].word_rect.left,
                   line_height);
      }
    }
    if (str[i] == ' ') { // New word
  ...
  if (str_len > 0) {
    ...
    last_line_break = store_last_word_break = last_word_break =
        last_stored_word_break = 0;
    MoveTo(dest_rect.left + 1,
           dest_rect.top + 1 + line_height * on_what_line + 9);
    for (i = 0; i < str_len; i++) {
      if (((str[i] != 39) && ((str[i] < 65) || (str[i] > 122)) &&
        ...
        if (((i - store_last_word_break >= 4) || (i >= str_len - 1)) &&
          ...
          store_words[current_rect].word_rect.left =
              ...
              (text_len[store_last_word_break] - text_len[last_line_break]) - 2;
          store_words[current_rect].word_rect.right =
              dest_rect.left + (text_len[i + 1] - text_len[last_line_break]) -
              1;
          store_words[current_rect].word_rect.top =
              dest_rect.top + 1 + line_height * on_what_line - 5;
          store_words[current_rect].word_rect.bottom =
      ...
      if (((text_len[i] - text_len[last_line_break] >
        ...
        if ((start_of_last_kept_word >= last_line_break) &&
            (current_rect > 0)) {
          OffsetRect(&store_words[current_rect - 1].word_rect,
                     5 + -1 * store_words[current_rect - 1].word_rect.left,
                     line_height);
