...
void rect_draw_some_item(HBITMAP src, RECT16 src_rect, HBITMAP dest,
                         RECT16 dest_rect, short trans, short main_win) {
...
HBITMAP load_pict(short pict_num, HDC) {
  ...
  case 705:
    ...
    break;
  case 800:
    if (party.stuff_done[SDF_USE_DARKER_GRAPHICS] == 0)
  ...
  case 830:
    ...
    break;
  case 831:
    got_bitmap = ReadBMP("images/STANIM.BMP");
    break;
  case 832:
    got_bitmap = ReadBMP("images/STARTBUT.BMP");
    break;
  case 850:
  ...
  case 860:
    ...
    break;
    //		case 875: got_bitmap = ReadBMP("images/DLOGMAPS.BMP"); break; => Exile
    //III maps
  case 880:
...
HBITMAP ReadScenFile(char const *filename) { // like ReadBMP but in the current
                                             // scenario folder only.
  ...
  strncat(str, party.scen_name,
          store_val +
              1); // the scenario is in a subfolder so add that to the filepath
  strcat(str, "exr/");
  strcat(str, filename);
...
// if terrain_to_draw >= 10000, force to draw graphic which is terrain_to_draw -
// 10000
{
  RECT where_draw;
  RECT source_rect;
  ...
  if (terrain_to_draw == -1) {
    ...
    Rectangle(hdc, where_draw.left, where_draw.top, where_draw.right,
              where_draw.bottom);
    SelectObject(hdc, store_bmp);
    if (!DeleteDC(hdc))
      DebugQuit("Cannot release DC 24");
    return;
  }
  if (terrain_to_draw >= 10000) { // force using a specific graphic
    ...
    source_rect =
        get_custom_rect(terrain_pic[terrain_to_draw] - 2000 + (anim_ticks % 4));
    anim_type = 0;
    terrain_there[i][j] = -1;
  ...
  if (dest == 0)
    rect_draw_some_item(source_gworld, source_rect, terrain_screen_gworld,
                        where_draw, (unsigned char)0, 0);
  else
    rect_draw_some_item(source_gworld, source_rect, terrain_screen_gworld,
                        where_draw, (unsigned char)0, 1);
}
    ...
    for (i = 0; i < 10; i++)
      if (party.out_c[i].exists == true) {
        if ((point_onscreen(party.p_loc, party.out_c[i].m_loc) == true) &&
          ...
          if (j == 7)
            party.out_c[i].exists = false; // begin watch out
          else
            picture_wanted =
                get_monst_picnum(party.out_c[i].what_monst.monst[j]);
          // end watch out
            ...
            if (picture_wanted >= 1000) {
              for (k = 0; k < width * height; k++) {
                source_rect = get_custom_rect(
                    picture_wanted % 1000 +
                    ((party.out_c[i].direction < 4) ? 0 : (width * height)) +
                    k);
                to_rect = monst_rects[(width - 1) * 2 + height - 1][k];
                rect_draw_some_item(spec_scen_g, source_rect, small_temp_gworld,
                                    to_rect, 0, 0);
                source_rect = to_rect;
                OffsetRect(&to_rect, 13 + 28 * where_draw.x,
                           13 + 36 * where_draw.y);
                rect_draw_some_item(small_temp_gworld, source_rect,
                                    terrain_screen_gworld, to_rect, 1, 0);
              }
            }
            if (picture_wanted < 1000) {
              for (k = 0; k < width * height; k++) {
                if (picture_wanted == 73 &&
                    k == 1) { // drake special case (split on two sheets)
                  source_gworld = monsters_gworld[4];
                  source_rect = get_monster_template_rect(
                      20, (party.out_c[i].direction < 4) ? 0 : 1, 0);
                } else if (picture_wanted == 101 &&
                           k == 1) { // bear special case (split on two columns)
                  source_gworld = monsters_gworld[5];
                  source_rect = get_monster_template_rect(
                      10, (party.out_c[i].direction < 4) ? 0 : 1, 0);
                } else {
                  source_gworld = monsters_gworld[m_pic_sheet[picture_wanted]];
                  source_rect = get_monster_template_rect(
                      party.out_c[i].what_monst.monst[j],
                      (party.out_c[i].direction < 4) ? 0 : 1, k);
                }
                to_rect = monst_rects[(width - 1) * 2 + height - 1][k];
                rect_draw_some_item(source_gworld, source_rect,
                                    small_temp_gworld, to_rect, 0, 0);
                source_rect = to_rect;
                OffsetRect(&to_rect, 13 + 28 * where_draw.x,
                           13 + 36 * where_draw.y);
                rect_draw_some_item(small_temp_gworld, source_rect,
  ...
  if (is_town())
    for (i = 0; i < T_M; i++)
      if ((c_town.monst.dudes[i].active != 0) &&
          (c_town.monst.dudes[i].m_d.spec_skill != MONSTER_INVISIBLE))
        if (party_can_see_monst(i)) {
          ...
          for (k = 0; k < width * height; k++) {
            ...
            if (c_town.monst.dudes[i].m_d.picture_num >= 1000) {
              source_rect = get_custom_rect(
                  (c_town.monst.dudes[i].m_d.picture_num % 1000) + k +
                  ((c_town.monst.dudes[i].m_d.direction < 4) ? 0
                                                             : width * height) +
                  ((combat_posing_monster == i + 100) ? (2 * width * height)
                                                      : 0));
              Draw_Some_Item(spec_scen_g, source_rect, terrain_screen_gworld,
                             store_loc, 1, 0);
            }
            if (c_town.monst.dudes[i].m_d.picture_num < 1000) {
              if (c_town.monst.dudes[i].m_d.picture_num == 73 &&
                  k == 1) { // drake special case (split on two sheets)
                source_gworld = monsters_gworld[4];
                source_rect = get_monster_template_rect(
                    20,
                    ((c_town.monst.dudes[i].m_d.direction < 4) ? 0 : 1) +
                        ((combat_posing_monster == i + 100) ? 10 : 0),
                    0);
              } else if (c_town.monst.dudes[i].m_d.picture_num == 101 &&
                         k == 1) { // bear special case (split on two columns)
                source_gworld = monsters_gworld[5];
                source_rect = get_monster_template_rect(
                    10,
                ...
                source_gworld = monsters_gworld
                    [m_pic_sheet[c_town.monst.dudes[i].m_d.picture_num]];
                source_rect = get_monster_template_rect(
              ...
              ter = t_d.terrain[c_town.monst.dudes[i].m_loc.x]
                               [c_town.monst.dudes[i].m_loc.y];
              // in bed?
              if ((store_loc.x >= 0) && (store_loc.x < 9) &&
                  (store_loc.y >= 0) && (store_loc.y < 9) &&
                  (scenario.ter_types[ter].picture == 143) &&
                  ((c_town.monst.dudes[i].m_d.m_type <
                    MONSTER_TYPE_DEMON) // is humanoïd ?
                   &&
                   (c_town.monst.dudes[i].m_d.m_type != MONSTER_TYPE_REPTILE) &&
                   (c_town.monst.dudes[i].m_d.m_type != MONSTER_TYPE_BEAST)) &&
                  ((c_town.monst.dudes[i].active == 1) ||
                   (monst_target[i] == 6)) &&
                  (width == 1) && (height == 1)) ////
                draw_one_terrain_spot((short)where_draw.x, (short)where_draw.y,
                                      10230, 0);
              else
                Draw_Some_Item(source_gworld, source_rect,
                               terrain_screen_gworld, store_loc, 1, 0);
            }
          }
        }
  if (is_combat()) {
  ...
  for (i = 0; i < NUM_OF_PCS; i++)
    if (adven[i].isAlive())
      if ((point_onscreen(center, pc_pos[i]) == true) &&
        ...
        if (mode == 0)
          Draw_Some_Item(pcs_gworld, source_rect, terrain_screen_gworld,
                         where_draw, 1, 0);
    ...
    if (t_i.items[i].variety != ITEM_TYPE_NO_ITEM) {
      ...
      if ((supressing_some_spaces == true) &&
        ...
        ;
               ...
               (party_can_see(t_i.items[i].item_loc) < 6)) {
        if (t_i.items[i].graphic_num >= 150) {
          ...
          rect_draw_some_item(spec_scen_g, source_rect, terrain_screen_gworld,
                              dest_rect, 1, 0);
        } else {
          ...
          rect_draw_some_item(
              (t_i.items[i].graphic_num < 45) ? items_gworld : tiny_obj_gworld,
...
void draw_outd_boats(location center) {
  ...
  for (i = 0; i < 30; i++)
    if ((point_onscreen(center, party.boats[i].boat_loc) == true) &&
        (party.boats[i].exists == true) &&
        (party.boats[i].which_town == INVALID_TOWN) &&
        (can_see(center, party.boats[i].boat_loc, 0) < 5) &&
        (party.in_boat != i)) {
      ...
      Draw_Some_Item(mixed_gworld, source_rect, terrain_screen_gworld,
                     where_draw, 1, 0);
    }
  for (i = 0; i < 30; i++)
    if ((point_onscreen(center, party.horses[i].horse_loc) == true) &&
        (party.horses[i].exists == true) &&
        (party.horses[i].which_town == INVALID_TOWN) &&
        (can_see(center, party.horses[i].horse_loc, 0) < 5) &&
        (party.in_horse != i)) {
        ...
        for (i = 0; i < 8; i++) {
          ...
          if (sfx[where_draw.x][where_draw.y] & flag)
            if (spot_seen[q][r] > 0) {
              ...
              Draw_Some_Item(fields_gworld, source_rect, terrain_screen_gworld,
                             loc, 1, 0);
            }
        }
      }
...
void draw_party_symbol(location center) {
  ...
  if ((party.in_boat < 0) && (party.in_horse < 0)) {
    ...
    if ((is_town()) &&
        (scenario.ter_types[t_d.terrain[c_town.p_loc.x][c_town.p_loc.y]]
             .picture == 143))
      draw_one_terrain_spot((short)target.x, (short)target.y, 10230, 0);
    else
      Draw_Some_Item(pcs_gworld, source_rect, terrain_screen_gworld, target, 1,
                     0);
  } else if (party.in_boat >= 0) {
    source_rect = boat_rects[dir_array[party.direction]];
    OffsetRect(&source_rect, 61, 0);
    Draw_Some_Item(mixed_gworld, source_rect, terrain_screen_gworld, target, 1,
                   0);
  } else {
...
RECT return_item_rect(short wanted) {
  ...
  if (wanted >= 400) {
    ter = wanted - 400;
    OffsetRect(&orig_rect, 112 * (ter / 5) + 28 * (anim_ticks % 4),
               36 * (ter % 5));
    return orig_rect;
  } else {
    ter = wanted % 50;
...
RECT get_monster_template_rect(unsigned char type_wanted, short mode,
                               short which_part)
// mode; // 0 - left  1 - right  +10 - combat mode
{
  ...
  if (mode >= 10)
    OffsetRect(&store_rect, 112 + 56 * (picture_wanted / 10) + 28 * (mode - 10),
               36 * ((picture_wanted % 10) + which_part));
  else
    OffsetRect(&store_rect, 56 * (picture_wanted / 10) + 28 * mode,
               36 * ((picture_wanted % 10) + which_part));
...
// mode; // 0 - town 1 - outdoor combat
{
  ...
  for (int x = 0; x < town_size[town_type]; x++)
    for (int y = 0; y < town_size[town_type]; y++)
      town_trim[x][y] = add_trim_to_array(
          x, y, (mode == 0) ? t_d.terrain[x][y] : combat_terrain[x][y]);
...
char add_trim_to_array(int x, int y, unsigned char ter_type) {
  ...
  if ((overall_mode == MODE_OUTDOORS) || (overall_mode == MODE_LOOK_OUTDOORS)) {
    ...
    if (x == town_size[town_type] - 1)
      at_right = true;
    if (y == town_size[town_type] - 1)
      at_bot = true;
  }
...
Boolean pt_in_rect(location loc, RECT16 rect) {
  if ((loc.x >= rect.left) && (loc.x <= rect.right) && (loc.y >= rect.top) &&
      (loc.y <= rect.bottom))
    return true;
  return false;
}
...
void adjust_monst_menu() {
  ...
  for (i = 0; i < 256; i++)
    if (on_monst_menu[i] >= 0) {
      ...
      sprintf((char *)monst_name, "%s",
              scen_item_list->monst_names[on_monst_menu[i]]);
      if ((total_added % 24 == 0) && (total_added > 0))
        InsertMenu(menu, 599,
                   MF_MENUBREAK | MF_BYCOMMAND | MF_ENABLED | MF_STRING,
                   600 + i, monst_name);
      else
        InsertMenu(menu, 599, MF_BYCOMMAND | MF_ENABLED | MF_STRING, 600 + i,
                   monst_name);
      total_added++;
    }
}
...
// mode;  // 0 - red   1 - green
{
  ...
  HDC hdc;
  HPEN hpen, old_pen;
  COLORREF x[3] = {RGB(200, 0, 0), RGB(102, 255, 0),
                   RGB(200, 0, 200)}; // RGB(204,204,204);
